...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        ...
        String routenumber = str.substring(1, seperate);
        firstpart = str.substring(seperate + 1, str.length());
        LatLons = firstpart.split(";");
        String name = LatLons[0];

        for (int lat = 1, lon = 2; lon < LatLons.length; lat += 2, lon += 2) {
            LatLon latlon = new LatLon(Double.parseDouble(LatLons[lat]), Double.parseDouble(LatLons[lon]));
            path.add(latlon);
        }

        storeRouteMap(routenumber, name, path);
    }
