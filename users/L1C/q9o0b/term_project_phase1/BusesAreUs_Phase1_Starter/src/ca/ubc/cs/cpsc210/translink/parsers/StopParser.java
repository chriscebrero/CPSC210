...
public class StopParser {
    ...
    public void parseStop(JSONObject stop) throws JSONException, StopDataMissingException {
        if (stop.has("StopNo") && stop.has("Name") && stop.has("Latitude") && stop.has("Longitude") && stop.has("Routes")) {
            ...
            Stop s = new Stop(StopNumber, name, latlon);
            List<Route> Routes = new ArrayList<>();
            for (int index = 0; index < routes.length; index++) {
                Route route = RouteManager.getInstance().getRouteWithNumber(routes[index]);
                route.addStop(s);
            }

            StopManager.getInstance().getStopWithId(StopNumber, name, latlon);
        } else {
